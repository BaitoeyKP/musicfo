...
function Discography() {
  ...
  return (
    <div className="bg-base-100 h-screen w-screen">
      ...
      <div className="pt-9 flex px-12 hd:px-24 justify-between">
        <Link to="/" className="text-4xl text-neutral flex items-center w-1/3">
          <span>{'<'}</span>
          <span className="hidden hd:block">&nbsp;All Artist</span>
        </Link>
        <h1 className="text-6xl font-bold text-center cursor-context-menu">DISCOGRAPHY</h1>
        <div className="w-1/3"></div>
      </div>
      <div className="px-24 py-9 grid grid-cols-1 m:grid-cols-2 l:grid-cols-3 hd:grid-cols-4 4k:grid-cols-5 place-items-center gap-y-9 gap-x-12 overflow-hidden">
        {album.map((x) => {
          return (
            <AlbumCard
              id={x.id}
